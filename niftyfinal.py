...
with tab1:
  ...
  if data!=None:
    ...
    if spot1>0:
      ...
      df1=df.copy()
      df1=df1.style.apply(highlight_second_highest,subset=['CALL_OI','PUT_OI','CALL_VOLUME','PUT_VOLUME','CALL_CHNG','PUT_CHNG','call_vol_per','put_vol_per']).map(color_two, subset=['STRIKE']).format(precision=0)
      ...
      st.dataframe(com, column_order=['call_oi','call_volume','STRIKE','put_oi','put_volume'])
  else:
